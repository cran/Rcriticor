critic <-
function (t, Y, fac = NULL, dinf = 10, durinf = 2, dsup = 90, dursup = 90, nperm = 0, nboot=0, period=365, dt=1,
seriesName="year", grtype = "image", roll = FALSE,alpha=0.05,ps.print=FALSE) 
{
    Rep = if (is.null(fac)) FALSE else TRUE
    m = dsup - dinf + 1
    p = dursup - durinf + 1
    z = matrix(0, m, p)
    n = length(Y)
    fac = if (Rep) fac else as.factor(1:n)
#===========================================================================================================================
#                            Header : parameters recall 
#===========================================================================================================================
    cat("Critical correlogram\n")
    cat("====================\n\n")
    cat("J.S.Pierre - 2015\n\n")
    cat("Data type: ",if(Rep) "With replicates\n\n" else "Without replicates\n\n")
...
#===========================================================================================================================
            ...
            for (k in 1:n) 
    {
        ...
        num = num - min(num) + 1
        for (i in dinf:dsup) for (j in durinf:dursup) {
            for (k in 1:n) S[i - dinf + 1, j - durinf + 1, k] = sum(t[(i + period * (num[k] - 1)):(i + j + period * (num[k] - 1))])
            z[i - dinf + 1, j - durinf + 1] = cor(Y, S[i - dinf + 1, j - durinf + 1, ])
        }
    }
    ...
    for (k in 1:nperm) {
        for (i in dinf:dsup) for (j in durinf:dursup) {
            ...
            if (abs(rho1) < abs(rho2)) 
                zB[i - dinf + 1, j - durinf + 1] = zB[i - dinf + 1, j - durinf + 1] + 1
        }
    }

    if (nperm != 0) zB = 1 - zB/nperm else zB = z * 0
    cat("\nNumber of random permutations: ",nperm,"\n")
    cat("Estimated p-value (random permutations):", if (nperm != 0) zB[debstar, durstar] else "NA", "\n")
...
if(nboot>0)
{
	...
	cat("====================\n\n")
    	...
    	{
		zBoot=z*0
    		fc=sample(fac,replace=TRUE)
    		YB = Y[as.numeric(fc)]
        	Sboot=S[,,as.numeric(fc)]
        	for (i in dinf:(dsup-1)) for (j in durinf:(dursup-1))  zBoot[i-dinf+1,j-durinf+1]=cor(YB, Sboot[i - dinf + 1,j - durinf+1, ])
	# Calculation of pseudovalues
	...
	cat("Bootstrap estimator for  beginning of the period:",debBoot,"\n")
	cat("Bootstrap estimator for  length of the period:",durBoot,"\n")
	serho=sqrt(var(pseudo[,1],na.rm=TRUE));cat("Standard error on rhomax :",serho,"\n")
	cat("\n Standard error for best correlation :",serho,"\n")
	cat("Confidence interval for best correlation (alpha =",alpha,"):\nLower : ",rhoBoot-qnorm(1-alpha/2)*serho, "\nUpper : ",rhoBoot+qnorm(1-alpha/2)*serho,"\n")  
	...
	A= if (nboot<50) qf(1-alpha,2,nboot-2)*2 else qchisq(1-alpha,2)
	...
	{
		X=x-mu[1]
		disc=X^2*MVCV[1,2]^2-MVCV[1,1]*(X^2*MVCV[2,2]-A*D)
		if (disc<0) disc=0
		y1=mu[2]+(X*MVCV[1,2]-sqrt(disc))/MVCV[1,1]
		y2=mu[2]+(X*MVCV[1,2]+sqrt(disc))/MVCV[1,1]
...
if (grtype == "contour") {
	dev.new()
        ...
        abline(debstar + dinf, 0)
        abline(intercept, -1, -1)
        points(durstar + durinf, debstar + dinf, pch = 19, bg = "red", cex = 2)
	if(nboot>0)
	{
	lines(ellips$x,ellips$y1)
	lines(ellips$x,ellips$y2)
...
#===========================================================================================================================
#                            Significance graphs in case of Monte-Carlo simulations
#===========================================================================================================================
    if (grtype == "filledcontour") 
	{
	dev.new()
        filled.contour(durinf:dursup, dinf:dsup, t(z), xlab = "span", 
            ylab = "start", color.palette = rainbow, plot.axes = 
		if(nboot==0) 
		{
                axis(1)
                axis(2)
